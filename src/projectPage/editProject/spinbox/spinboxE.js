...
S.extend(SpinBox, Base,  /** @lends SpinBox.prototype*/{
     ...
     */
    enable:function(){
        $('.ks-spinbox-sign').attr('style','visibility:none')

    },
    disable:function(){
        $('.ks-spinbox-sign').attr('style','visibility:hidden')
    ...
    _addHTML: function(){
        ...
        $target.each(function(item){
            ...
            var $parent = item.parent(),
                ...
                $plusEl = $('<a class="'+ getCls.plus + ' ' + getCls.sign +'" role="button" style="visibility:'+isHide+'">+</a>'),
                $minusEl = $('<a class="'+ getCls.minus + ' ' + getCls.sign +'"  role="button"  style="visibility:'+isHide+'">-</a>');
    ...
    _formatNum : function($target){
        var self = this, min = Number($target.attr('data-min')),
    ...
    _limitRange : function(value, target){
        var self = this, $target = target, $parent = $target.parent(1), _toFloat = self._toFloat,getCls = self.get('cls'),
            min = _toFloat($target.attr('data-min')),
            max = _toFloat($target.attr('data-max')) || 1,
            inputValue = Math.max(min, value);
